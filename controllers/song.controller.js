...
async function createSong(req, res, next) {
	try {
		...
		const thumbnailRes = await uploadAsset(
			path.resolve(req.files["thumbnail"][0].path),
			"image"
		);
		const audioRes = await uploadAsset(
			path.resolve(req.files["audio"][0].path),
			"video"
		);
		...
		let newSong = new Song({
			...
			downloadUrl: await getAssetAttachment(
				audioRes.public_id,
...
async function deleteSong(req, res, next) {
	try {
		const { id } = req.params;
		const deletedSong = await Song.findByIdAndDelete(id).populate([
			"audio",
			"thumbnail",
		]);
		if (!deletedSong) {
